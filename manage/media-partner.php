...
<html>
<head>
<title>Add Media Partner | Assessny Dashboard</title>
...
<h1 style="text-transform: uppercase;font-weight: 200;font-size: 2em;margin-bottom:5%">Add Media Partner</h1>
<form method="POST" enctype="multipart/form-data">
    <input type="file" name="picture-upload" style="display: block;" required>
    <input type="submit" name="upload" class="btn btn-outline-success" value="Upload Media Partner" style="margin-top: 5%;">
</form>
<?php

if(isset($_POST['upload'])) {
$targetDir = "../media-partners/";
...
if($main_pic_name !== "" && move_uploaded_file($_FILES["picture-upload"]["tmp_name"], $targetFilePathMain)) {
    $query_select = mysqli_query($connect, "SELECT * FROM media_partners WHERE pic_name = '$main_pic_name'");
    if(mysqli_num_rows($query_select) > 0) {
        echo "<div class='alert alert-danger'>You added it before</div>";
    } else {
        $sql = "INSERT INTO media_partners (pic_name) VALUES ('$main_pic_name')";
